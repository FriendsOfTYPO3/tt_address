...
class AddressRepositoryTest extends FunctionalTestCase
{
    ...
    public function rawQueryReturnsCorrectQuery()
    {
        ...
        if ((new Typo3Version())->getMajorVersion() >= 12) {
            $sql = 'SELECT `tt_address`.* FROM `tt_address` `tt_address` WHERE (((((`tt_address`.`pid` IN (1, 2)) AND ( NOT(((`tt_address`.`latitude` IS NULL) OR (`tt_address`.`latitude` = 0)))))) AND ( NOT(((`tt_address`.`longitude` IS NULL) OR (`tt_address`.`longitude` = 0)))))) AND (`tt_address`.`sys_language_uid` IN (0, -1)) AND (`tt_address`.`t3ver_oid` = 0) AND (((`tt_address`.`hidden` = 0) AND (`tt_address`.`starttime` <= 1714554720) AND (((`tt_address`.`endtime` = 0) OR (`tt_address`.`endtime` > 1714554720)))) AND tt_address.deleted=0)';
        }
